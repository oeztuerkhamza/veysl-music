...
/**
 * City-specific FAQ (2–3 pairs, never the full /ablauf FAQ). Uses native
 * <details>/<summary> — accessible and keyboard-operable without any JS,
 * matching "Server Components by default" / "'use client' only where truly
 * needed".
 */
export function CityFaq({ city, locale }: { city: City; locale: Locale }) {
  const t = useTranslations('city');

  const entries = city.faq
    .map((entry) => ({
  ...
  return (
    <Section id="faq">
      <Container>
        <Reveal>
          <SectionHeading eyebrow={t('faq.eyebrow')} title={t('faq.title', { city: city.name })} />
        </Reveal>
